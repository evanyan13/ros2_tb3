...
with open("/home/evanyan13/colcon_ws/src/autonomous_exploration/config/params.yaml", 'r') as file:
...
# Heuristic function for A* algorithm, calculating Euclidean distance
def heuristic(a, b):
...
# B-spline path smoothing function
...
def exploration(data,width,height,resolution,column,row,originX,originY):
        ...
        if len(groups) == 0: #Grup yoksa exploration_active tamamlandı
            path = -1
...
class navigationControl(Node):
    def __init__(self):
        ...
        print("[INFO] ACTIVATE EXPLORATION MODE")
        self.exploration_active = True
        threading.Thread(target=self.exp).start() # Start exploration function in new thread.
        
    def exp(self):
        twist = Twist()
        while True: # When sensor data is available
            if not hasattr(self,'map_data') or not hasattr(self,'odom_data') or not hasattr(self,'scan_data'):
                time.sleep(0.1)
                continue
            if self.exploration_active == True:
                ...
                t = t - 0.2 #x = v * t formülüne göre hesaplanan sureden 0.2 saniye cikarilir. t sure sonra exploration_active fonksiyonu calistirilir.
                self.t = threading.Timer(t,self.target_callback) #Hedefe az bir sure kala exploration_active fonksiyonunu calistirir.
                self.t.start()
            ...
            # Path Tracking Block
                ...
                if(abs(self.x - self.path[-1][0]) < target_error and abs(self.y - self.path[-1][1]) < target_error):
                    ...
                    w = 0.0
                    self.exploration_active = True
                    print("[INDO] TARGET REACHED")
                    self.t.join() # Wait until the thread is finished
                twist.linear.x = v
                twist.angular.z = w
                self.publisher.publish(twist)
                time.sleep(0.1)
            # End of Path Tracking Block
